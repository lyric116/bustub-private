...
ReadPageGuard::ReadPageGuard(ReadPageGuard &&that) noexcept
    : page_id_(that.page_id_),
      ...
      has_page_latch_(that.has_page_latch_) {
  that.page_id_ = INVALID_PAGE_ID;
  that.is_valid_ = false;
  that.has_page_latch_ = false;
}
...
void ReadPageGuard::Flush() {
  ...
  requests.push_back(DiskRequest{/*is_write=*/true, frame_->GetDataMut(), page_id_, std::move(promise)});
...
void ReadPageGuard::Drop() {
  ...
  if (has_page_latch_) {
    ...
    has_page_latch_ = false;
  }

  {
    std::scoped_lock guard(*bpm_latch_);
    const auto old_pin = frame_->pin_count_.fetch_sub(1);
    BUSTUB_ENSURE(old_pin > 0, "read guard drop encountered negative pin count");
    if (old_pin == 1) {
...
void WritePageGuard::Drop() {
...

  {
    std::scoped_lock guard(*bpm_latch_);
    const auto old_pin = frame_->pin_count_.fetch_sub(1);
    BUSTUB_ENSURE(old_pin > 0, "write guard drop encountered negative pin count");
    if (old_pin == 1) {
      replacer_->SetEvictable(frame_->frame_id_, true);
    }
  }
